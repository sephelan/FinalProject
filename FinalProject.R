...
transfinalmodel <- lm(sqrt(stream$max90)~stream$DRAIN_SQKM+stream$MAR_PPT7100_CM+stream$T_AVG_BASIN)
res_tran <- residuals((transfinalmodel))
qqnorm(res_tran)
qqline(res_tran)
shapiro.test(res_tran)
